...
import { getFreightGroupsByDriver, getPendingFreightTotal, getPaidFreightTotal, getTotalFreight, UnifiedFreightRecord } from '../utils/freightUtils';
...
export const TableMotoristas: React.FC<TableMotoristasProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<'FREIGHTS' | 'DRIVERS'>('FREIGHTS');
  ...
  // Group freights by unique driver using centralized freightUtils
  const freightGroups = React.useMemo(() => {
  ...
  const grandTotalFreight = getTotalFreight({
    Cargas: cargas,
    Vendas: vendas,
    Motoristas: motoristas,
    appSettings: { freightRatePerTon },
  ...
  const grandPendingFreight = getPendingFreightTotal({
    Cargas: cargas,
    Vendas: vendas,
    Motoristas: motoristas,
    appSettings: { freightRatePerTon },
  ...
  const grandPaidFreight = getPaidFreightTotal({
    Cargas: cargas,
    Vendas: vendas,
    Motoristas: motoristas,
    appSettings: { freightRatePerTon },
  ...
  return (
    <div className="space-y-4">
      ...
      {/* TAB CONTENT: FREIGHTS BY DRIVER */}
      {activeTab === 'FREIGHTS' && (
        <div className="space-y-4">
          {pdfNotification && (
          ...
          {freightGroups.length === 0 ? (
            <div className="glass-card-static p-8 text-center text-slate-500 font-medium">
              Nenhum lançamento de frete encontrado.
            </div>
          ) : (
